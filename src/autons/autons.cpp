...
void autoSKILLS() {
    ...
    // // match loader 1
    // std::cout << "-----------------------------------------" << std::endl;
    // chassis.move(33); std::cout << (chassis.get_left_position_in() + chassis.get_right_position_in()) / 2 << std::endl;
    ...
    double dist2 = dist.objectDistance(inches); std::cout << "DISTANCE: " << dist2 << std::endl; chassis.move(18.8 - dist2); //20.3-dist2
    chassis.turn(0);
    auto task6 = []() {scraper.set(true); intake();}; thread t_task6 = thread(task6);
    chassis.move(28, 4);
    wait(1500, msec);
    auto task7 = []() { hood.set(true); wait(2000, msec); trapdoor.set(true); moveIntake(-12, -12); }; thread t_task7 = thread(task7);
    chassis.lin_max = 6; chassis.setLinPID(12, 1, 0, 7, 0); chassis.setThetaPID(6, 0.4, 0, 1, 0); chassis.lin_timeout = 2000; chassis.arc(-28.75, 1.5);//91.5
    wait(1800, msec);
