...
void autoSKILLS() {
    ...
    // chassis.lin_max = 6; chassis.setLinPID(12, 1, 0, 7, 0); chassis.setThetaPID(6, 0.4, 0, 1, 0); chassis.lin_timeout = 2000; chassis.move(-28.75);//91.5
    // wait(1800, msec);
    chassis.move(-10); 
    scraper.set(false);
    wait(200, msec);
    chassis.turn(90); std::cout << imu.rotation() << " " << chassis.get_absolute_heading() - 360 << std::endl;

    // arc code
    chassis.setLinPID(12, 1, 0, 7, 0); chassis.setThetaPID(10.5, 0.4, 0, 1, 0);
    auto task8 = []() {descorer.set(true);}; thread t_task8 = thread(task8);
    chassis.arc(75, 169.5); //84.5 init
    chassis.setLinPID(12, 1, 0, 7, 0); chassis.setThetaPID(8.5, 0.4, 0, 1, 0); chassis.arc(24, -92);
