...
{
	if (cont.ButtonUp.pressing() == true)
	{
		return btnUP;
	}
	if (cont.ButtonDown.pressing() == true)
	{
		return btnDOWN;
	}
	if (cont.ButtonLeft.pressing() == true)
	{
		return btnLEFT;
	}
	if (cont.ButtonRight.pressing() == true)
	{
		return btnRIGHT;
	}
	if (cont.ButtonA.pressing() == true)
	{
		return btnA;
	}
	if (cont.ButtonB.pressing() == true)
	{
		return btnB;
	}
	if (cont.ButtonX.pressing() == true)
	{
		return btnX;
	}
	if (cont.ButtonY.pressing() == true)
	{
		return btnY;
	}
	if (cont.ButtonR1.pressing() == true)
	{
		return btnR1;
	}
	if (cont.ButtonR2.pressing() == true)
	{
		return btnR2;
	}
	if (cont.ButtonL1.pressing() == true)
	{
		return btnL1;
	}
	if (cont.ButtonL2.pressing() == true)
	{
		return btnL2;
	}
	else
...
void ACCESS_OS::clearLine(int l_row) //select what row to clear but not selector if isCursorOn = true
{
	if (isCursorOn == true)
	{
		cont.Screen.setCursor(l_row + 1, 2);	//Prevent overwrite of selector GUI
		cont.Screen.print("               "); //15 spaces (1 less than screenTextWidth)
	}
	else
	{
		cont.Screen.clearLine();	//If isCursorOn = false, wipe line with system clearLine
	}
}
...
void ACCESS_OS::clearScreen() //clears everything but not selector if isCursorOn = true
{
	...
	{
		cont.Screen.clearScreen();	//If isCursorOn = false, wipe line with system clearScreen
	}
...
void ACCESS_OS::print(std::string text, int row, int col) //pass text, row, and col
{
	...
	if (isCursorOn == true)
	{
		cont.Screen.setCursor(row + 1, col + 2); //Col + 2: Prevent overwrite of selector GUI
	}
	else
	{
		cont.Screen.setCursor(row + 1, col + 1); //Col + 1: Start from beginning
	}
	cont.Screen.print(text.c_str()); //Future GUI will scroll text
}
