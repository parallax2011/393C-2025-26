...
int SidewaysTracker_port, float SidewaysTracker_diameter, float SidewaysTracker_center_distance) :
  ...
  drive_in_toDeg_ratio(wheel_ratio/360.0*M_PI*wheel_diameter),
  ForwardTracker_center_distance(ForwardTracker_center_distance),
  ForwardTracker_diameter(ForwardTracker_diameter),
  ForwardTracker_in_toDeg_ratio(M_PI*ForwardTracker_diameter/360.0),
  SidewaysTracker_center_distance(SidewaysTracker_center_distance),
  SidewaysTracker_diameter(SidewaysTracker_diameter),
  SidewaysTracker_in_toDeg_ratio(M_PI*SidewaysTracker_diameter/360.0),
  drive_setup(drive_setup),
  DriveL(DriveL),
  DriveR(DriveR),
  Gyro(inertial(gyro_port)),
  DriveLF(abs(DriveLF_port), isReversed(DriveLF_port)),
  DriveRF(abs(DriveRF_port), isReversed(DriveRF_port)),
  DriveLB(abs(DriveLB_port), isReversed(DriveLB_port)),
  DriveRB(abs(DriveRB_port), isReversed(DriveRB_port)),
  R_ForwardTracker(ForwardTracker_port),
  R_SidewaysTracker(SidewaysTracker_port),
  E_ForwardTracker(ThreeWire.Port[toPort(ForwardTracker_port)]),
  E_SidewaysTracker(ThreeWire.Port[toPort(SidewaysTracker_port)])
{
...
void Drive::setTurn(float turn_max, float turn_kp, float turn_ki, float turn_kd, float turn_starti){
...
void Drive::setAng(float ang_max, float ang_kp, float ang_ki, float ang_kd, float ang_starti){
...
float Drive::getTheta(){  // return Gyro scale-corrected heading in the range [0, 360).
  return( reduce_0_360( Gyro.rotation()*360.0/gyro_scale ) ); 
}

float Drive::getLeftPosition(){
  return( DriveL.position(deg)*drive_in_toDeg_ratio );
}

float Drive::getRightPosition(){
  return( DriveR.position(deg)*drive_in_toDeg_ratio );
}

float Drive::getAvgPosition(){
    return( (getLeftPosition() + getRightPosition()) / 2.0 );
}

void Drive::brake(vex::brakeType mode){
  DriveL.stop(mode);
  DriveR.stop(mode);
}
 ...
 * current heading of the drive.
 * 
 * @param angle Desired angle in degrees.
 */

void Drive::turn(float angle){
  turn_to_angle(angle, turn_max, turn_settle_error, turn_settle_time, turn_timeout, turn_kp, turn_ki, turn_kd, turn_starti);
}

void Drive::turn(float angle, float max) {
  turn_to_angle(angle, max, turn_settle_error, turn_settle_time, turn_timeout, turn_kp, turn_ki, turn_kd, turn_starti);
}

void Drive::turn(float angle, float max, float timeout) {
  turn_to_angle(angle, max, turn_settle_error, turn_settle_time, timeout, turn_kp, turn_ki, turn_kd, turn_starti);
}

void Drive::turn(float angle, float max, float settleError, float settleTime, float timeout) {
  turn_to_angle(angle, max, settleError, settleTime, timeout, turn_kp, turn_ki, turn_kd, turn_starti);
}
...
void Drive::turn_to_angle(float angle, float turn_max, float turn_settle_error, float turn_settle_time, float turn_timeout, float turn_kp, float turn_ki, float turn_kd, float turn_starti){
  pid pid_turn(reduce_neg180_180(angle - getTheta()), turn_kp, turn_ki, turn_kd, turn_starti, turn_settle_error, turn_settle_time, turn_timeout);
  while( !pid_turn.isSettled() ){
    float error = reduce_neg180_180(angle - getTheta());
    float output = pid_turn.compute(error);
    output = restrict(output, -turn_max, turn_max);
    spinVolts(output, -output);
    //std::cout << "Ang: " << getTheta() << std::endl;
    task::sleep(10);
  }
//   std::cout << "Ang: " << getTheta() << std::endl;
}

void Drive::kTurn(float angle, float turn_max, float turn_kp, float turn_ki, float turn_kd, float turn_starti) {
  turn_to_angle(angle, turn_max, turn_settle_error, turn_settle_time, turn_timeout, turn_kp, turn_ki, turn_kd, turn_starti);
}     
...
void Drive::move(float distance){
  drive_distance(distance, getTheta(), lin_max, ang_max, lin_settle_error, lin_settle_time, lin_timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}

// void Drive::arc(float distance, float heading, const ArcParams& p) {
//   drive_distance( distance, heading, p.linMax, p.angMax, lin_settle_error, lin_settle_time, p.timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
// }


void Drive::arc(float distance, float heading){
  drive_distance(distance, heading, lin_max, ang_max, lin_settle_error, lin_settle_time, lin_timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}

void Drive::arc(float distance, float heading, float timeout){
  drive_distance(distance, heading, lin_max, ang_max, lin_settle_error, lin_settle_time, timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}


void Drive::arc(float distance, float heading, float max, float thetaMax, float timeout){
  drive_distance(distance, heading, max, thetaMax, lin_settle_error, lin_settle_time, timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}

void Drive::move(float distance, float max_voltage) {
  drive_distance(distance, getTheta(), max_voltage, ang_max, lin_settle_error, lin_settle_time, lin_timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}

void Drive::move(float distance, float max_voltage, float timeout) {
  drive_distance(distance, getTheta(), max_voltage, ang_max, lin_settle_error, lin_settle_time, timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}

void Drive::move(float distance, float max_voltage, float settleError, float settleTime, float timeout) {
  drive_distance(distance, getTheta(), max_voltage, ang_max, settleError, settleTime, timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}
...
void Drive::drive_distance(float distance, float heading, float lin_max, float ang_max, float lin_settle_error, float lin_settle_time, float lin_timeout, float lin_kp, float lin_ki, float lin_kd, float lin_starti, float ang_kp, float ang_ki, float ang_kd, float ang_starti){
  pid pid_lin(distance, lin_kp, lin_ki, lin_kd, lin_starti, lin_settle_error, lin_settle_time, lin_timeout);
  pid pid_theta(reduce_neg180_180(heading - getTheta()), ang_kp, ang_ki, ang_kd, ang_starti);
  float startAvgPosition = (getLeftPosition() + getRightPosition()) / 2.0;
  float avgPosition = startAvgPosition;

  while(pid_lin.isSettled() == false){

    avgPosition = (getLeftPosition()+getRightPosition())/2.0;
    float lin_error = distance + startAvgPosition - avgPosition;
    
    float theta_error = reduce_neg180_180(heading - getTheta());
    float drive_output = pid_lin.compute(lin_error);
    float heading_output = pid_theta.compute(theta_error);

    drive_output = restrict(drive_output, -lin_max, lin_max);
    heading_output = restrict(heading_output, -ang_max, ang_max);
...
void Drive::swingLeft(float angle){
  swingLeft(angle, swing_max, swing_settle_error, swing_settle_time, swing_timeout, swing_kp, swing_ki, swing_kd, swing_starti);
}

void Drive::swingLeft(float angle, float swing_max, float swing_settle_error, float swing_settle_time, float swing_timeout, float swing_kp, float swing_ki, float swing_kd, float swing_starti){
  pid swingPID(reduce_neg180_180(angle - getTheta()), swing_kp, swing_ki, swing_kd, swing_starti, swing_settle_error, swing_settle_time, swing_timeout);
...
void Drive::swingRight(float angle){
  swingRight(angle, swing_max, swing_settle_error, swing_settle_time, swing_timeout, swing_kp, swing_ki, swing_kd, swing_starti);
}

void Drive::swingRight(float angle, float swing_max, float swing_settle_error, float swing_settle_time, float swing_timeout, float swing_kp, float swing_ki, float swing_kd, float swing_starti){
  pid swingPID(reduce_neg180_180(angle - getTheta()), swing_kp, swing_ki, swing_kd, swing_starti, swing_settle_error, swing_settle_time, swing_timeout);
  while(swingPID.isSettled() == false){
    float error = reduce_neg180_180(angle - getTheta());
    float output = swingPID.compute(error);
...
float Drive::get_ForwardTracker_position(){
  ...
  if (drive_setup==TANK_ONE_FORWARD_ENCODER || drive_setup == TANK_TWO_ENCODER || drive_setup == HOLONOMIC_TWO_ENCODER){
    return(E_ForwardTracker.position(deg)*ForwardTracker_in_toDeg_ratio);
  }else{
    return(R_ForwardTracker.position(deg)*ForwardTracker_in_toDeg_ratio);
  }
}
...
float Drive::get_SidewaysTracker_position(){
  if (drive_setup==TANK_ONE_FORWARD_ENCODER || drive_setup == TANK_ONE_FORWARD_ROTATION || drive_setup == ZERO_TRACKER_ODOM){
    ...
    return(E_SidewaysTracker.position(deg)*SidewaysTracker_in_toDeg_ratio);
  }else{
    return(R_SidewaysTracker.position(deg)*SidewaysTracker_in_toDeg_ratio);
  }
}
...
void Drive::position_track(){
  while(1){
    odom.updatePosition(get_ForwardTracker_position(), get_SidewaysTracker_position(), getTheta());
    task::sleep(5);
  }
}
...
void Drive::drive_to_point(float X_position, float Y_position){
  drive_to_point(X_position, Y_position, lin_min, lin_max, ang_max, lin_settle_error, lin_settle_time, lin_timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}
...
void Drive::drive_to_point(float X_position, float Y_position, float lin_min, float lin_max, float ang_max, float lin_settle_error, float lin_settle_time, float lin_timeout, float lin_kp, float lin_ki, float lin_kd, float lin_starti, float ang_kp, float ang_ki, float ang_kd, float ang_starti){
  ...
  float start_angle_deg = toDeg(atan2(X_position-get_X_position(),Y_position-get_Y_position()));
  pid headingPID(start_angle_deg-getTheta(), ang_kp, ang_ki, ang_kd, ang_starti);
  bool line_settled = false;
  bool prev_line_settled = isLineSettled(X_position, Y_position, start_angle_deg, get_X_position(), get_Y_position());
  while(!drivePID.isSettled()){
    line_settled = isLineSettled(X_position, Y_position, start_angle_deg, get_X_position(), get_Y_position());
    if(line_settled && !prev_line_settled){ break; }
    prev_line_settled = line_settled;

    float drive_error = hypot(X_position-get_X_position(),Y_position-get_Y_position());
    float heading_error = reduce_neg180_180(toDeg(atan2(X_position-get_X_position(),Y_position-get_Y_position()))-getTheta());
    float drive_output = drivePID.compute(drive_error);

    float heading_scale_factor = cos(toRad(heading_error));
    ...
    drive_output = restrict(drive_output, -fabs(heading_scale_factor)*lin_max, fabs(heading_scale_factor)*lin_max);
    heading_output = restrict(heading_output, -ang_max, ang_max);

    drive_output = restrictMinVoltage(drive_output, lin_min);

    spinVolts(leftVoltageScaling(drive_output, heading_output), rightVoltageScaling(drive_output, heading_output));
    task::sleep(10);
  }
}
...
void Drive::drive_to_pose(float X_position, float Y_position, float angle){
  drive_to_pose(X_position, Y_position, angle, boomerang_lead, boomerang_setback, lin_min, lin_max, ang_max, lin_settle_error, lin_settle_time, lin_timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}

void Drive::drive_to_pose(float X_position, float Y_position, float angle, float lead, float setback, float lin_min){
  drive_to_pose(X_position, Y_position, angle, lead, setback, lin_min, lin_max, ang_max, lin_settle_error, lin_settle_time, lin_timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}
...
void Drive::drive_to_pose(float X_position, float Y_position, float angle, float lead, float setback, float lin_min, float lin_max, float ang_max, float lin_settle_error, float lin_settle_time, float lin_timeout, float lin_kp, float lin_ki, float lin_kd, float lin_starti, float ang_kp, float ang_ki, float ang_kd, float ang_starti){
  ...
  pid headingPID(toDeg(atan2(X_position-get_X_position(),Y_position-get_Y_position()))-getTheta(), ang_kp, ang_ki, ang_kd, ang_starti);
  bool line_settled = isLineSettled(X_position, Y_position, angle, get_X_position(), get_Y_position());
  bool prev_line_settled = isLineSettled(X_position, Y_position, angle, get_X_position(), get_Y_position());
  bool crossed_center_line = false;
  bool center_line_side = isLineSettled(X_position, Y_position, angle+90, get_X_position(), get_Y_position());
  bool prev_center_line_side = center_line_side;
  while(!drivePID.isSettled()){
    line_settled = isLineSettled(X_position, Y_position, angle, get_X_position(), get_Y_position());
    if(line_settled && !prev_line_settled){ break; }
    prev_line_settled = line_settled;

    center_line_side = isLineSettled(X_position, Y_position, angle+90, get_X_position(), get_Y_position());
    ...
    float carrot_X = X_position - sin(toRad(angle)) * (lead * target_distance + setback);
    float carrot_Y = Y_position - cos(toRad(angle)) * (lead * target_distance + setback);

    float drive_error = hypot(carrot_X-get_X_position(),carrot_Y-get_Y_position());
    float heading_error = reduce_neg180_180(toDeg(atan2(carrot_X-get_X_position(),carrot_Y-get_Y_position()))-getTheta());

    if (drive_error<lin_settle_error || crossed_center_line || drive_error < setback) { 
      heading_error = reduce_neg180_180(angle-getTheta()); 
...
void Drive::turn_to_point(float X_position, float Y_position){
  turn_to_point(X_position, Y_position, 0, turn_max, turn_settle_error, turn_settle_time, turn_timeout, turn_kp, turn_ki, turn_kd, turn_starti);
}

void Drive::turn_to_point(float X_position, float Y_position, float extra_angle_deg){
  turn_to_point(X_position, Y_position, extra_angle_deg, turn_max, turn_settle_error, turn_settle_time, turn_timeout, turn_kp, turn_ki, turn_kd, turn_starti);
}
...
void Drive::turn_to_point(float X_position, float Y_position, float extra_angle_deg, float turn_max, float turn_settle_error, float turn_settle_time, float turn_timeout, float turn_kp, float turn_ki, float turn_kd, float turn_starti){
  pid turnPID(reduce_neg180_180(toDeg(atan2(X_position-get_X_position(),Y_position-get_Y_position())) - getTheta()), turn_kp, turn_ki, turn_kd, turn_starti, turn_settle_error, turn_settle_time, turn_timeout);
  while(turnPID.isSettled() == false){
    float error = reduce_neg180_180(toDeg(atan2(X_position-get_X_position(),Y_position-get_Y_position())) - getTheta() + extra_angle_deg);
...
void Drive::holonomic_drive_to_pose(float X_position, float Y_position){
  holonomic_drive_to_pose(X_position, Y_position, getTheta(), lin_max, ang_max, lin_settle_error, lin_settle_time, lin_timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}

void Drive::holonomic_drive_to_pose(float X_position, float Y_position, float angle){
  holonomic_drive_to_pose(X_position, Y_position, angle, lin_max, ang_max, lin_settle_error, lin_settle_time, lin_timeout, lin_kp, lin_ki, lin_kd, lin_starti, ang_kp, ang_ki, ang_kd, ang_starti);
}
...
void Drive::holonomic_drive_to_pose(float X_position, float Y_position, float angle, float lin_max, float ang_max, float lin_settle_error, float lin_settle_time, float lin_timeout, float lin_kp, float lin_ki, float lin_kd, float lin_starti, float ang_kp, float ang_ki, float ang_kd, float ang_starti){
  pid drivePID(hypot(X_position-get_X_position(),Y_position-get_Y_position()), lin_kp, lin_ki, lin_kd, lin_starti, lin_settle_error, lin_settle_time, lin_timeout);
  pid turnPID(angle-getTheta(), ang_kp, ang_ki, ang_kd, ang_starti, turn_settle_error, turn_settle_time, turn_timeout);
  while( !(drivePID.isSettled() && turnPID.isSettled()) ){
    float drive_error = hypot(X_position-get_X_position(),Y_position-get_Y_position());
    float turn_error = reduce_neg180_180(angle-getTheta());

    float drive_output = drivePID.compute(drive_error);
    float turn_output = turnPID.compute(turn_error);

    drive_output = restrict(drive_output, -lin_max, lin_max);
    turn_output = restrict(turn_output, -ang_max, ang_max);

    float heading_error = atan2(Y_position-get_Y_position(), X_position-get_X_position());

    DriveLF.spin(fwd, drive_output*cos(toRad(getTheta()) + heading_error - M_PI/4) + turn_output, volt);
    DriveLB.spin(fwd, drive_output*cos(-toRad(getTheta()) - heading_error + 3*M_PI/4) + turn_output, volt);
    DriveRB.spin(fwd, drive_output*cos(toRad(getTheta()) + heading_error - M_PI/4) - turn_output, volt);
    DriveRF.spin(fwd, drive_output*cos(-toRad(getTheta()) - heading_error + 3*M_PI/4) - turn_output, volt);
    task::sleep(10);
  }
}
...
void Drive::arcade(int deadzone, bool curve, int power) {
  ...
  DriveL.spin(fwd, toVolt(throttle + turn), volt);
  DriveR.spin(fwd, toVolt(throttle - turn), volt);
}
...
void Drive::tank(){
  ...
  DriveL.spin(fwd, toVolt(leftthrottle), volt);
  DriveR.spin(fwd, toVolt(rightthrottle), volt);
}
